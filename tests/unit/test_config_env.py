...
# 鍔熻兘锛氶獙璇?TOML 涓?agent.mode=plan_execute 鑳藉啓鍏ラ厤缃?
# 璁捐锛氫娇鐢?KAMA_CONFIG 鎸囧悜涓存椂 TOML锛岄伩鍏ㄥ眬閰嶇疆骞叉壈
def test_agent_mode_loaded_from_toml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "kama.toml"
    toml_path.write_bytes(b'[agent]\nmode = "plan_execute"\n')
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KAMA_CONFIG", str(toml_path))
    monkeypatch.delenv("KAMA_AGENT_MODE", raising=False)

    cfg = get_config()

    assert cfg.agent.mode == "plan_execute"


# 鍔熻兘锛氶獙璇?KAMA_AGENT_MODE 鐜鍙橀噺鑳借鐩?TOML agent.mode
# 璁捐锛歍OML 鍐?react锛岀幆澧冨彉閲忓啓 plan_execute锛岀‘璁ゆ渶楂樹紭鍏堢骇鐢熸晥
def test_agent_mode_env_overrides_toml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "kama.toml"
    toml_path.write_bytes(b'[agent]\nmode = "react"\n')
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KAMA_CONFIG", str(toml_path))
    monkeypatch.setenv("KAMA_AGENT_MODE", "plan_execute")
