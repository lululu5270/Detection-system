...
from kama_claude.core.plan_execute import PlanExecuteLoop
...
def _make_plan_loop(
    ...
    registry: ToolRegistry | None = None,
    bus: EventBus | None = None,
) -> tuple[PlanExecuteLoop, EventBus]:
    b = bus or EventBus()
    return PlanExecuteLoop(provider, registry or ToolRegistry(), b), b  # type: ignore[arg-type]
...
# 鍔熻兘锛氶獙璇?PlanExecuteLoop 鍏堝皢璁″垝鍐欏叆 context锛屽啀杩涘叆甯歌鎵ц寰幆
# 璁捐锛氱敤涓ゅ段 provider 响应 [plan, end_turn]，断言 context 中存在计划消息且最终成功
async def test_plan_execute_generates_plan_before_execution() -> None:
    provider = _MockProvider([
        LlmResponse(stop_reason="end_turn", text="1. Inspect files"),
        LlmResponse(stop_reason="end_turn", text="done"),
    ])
    loop, _ = _make_plan_loop(provider)
    ctx = _ctx()
    await loop.run(ctx)
    assert ctx.status == "success"
    assert ctx.step == 1
    assert "Execution plan" in str(ctx.messages[1]["content"])
    assert ctx.messages[2]["role"] == "user"


# 鍔熻兘锛氶獙璇?PlanExecuteLoop 鍦ㄨ鍒掗樁娈靛け璐ユ椂鏍囪 plan_llm_error
# 璁捐锛氳 provider 棣栨璋冪敤鎶涘紓甯革紝纭涓嶈繘鍏ュ悗缁墽琛岄樁娈?
async def test_plan_execute_plan_error_marks_failed() -> None:
    provider = _MockProvider([], exc=RuntimeError("plan api error"))
    loop, _ = _make_plan_loop(provider)
    ctx = _ctx()
    await loop.run(ctx)
    assert ctx.status == "failed"
    assert ctx.reason == "plan_llm_error"
    assert ctx.step == 0
