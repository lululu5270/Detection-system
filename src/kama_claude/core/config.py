...
def _apply_toml(config: KamaConfig, data: dict[str, Any]) -> None:
    ...
    if "agent" in data:
        ...
        unknown_agent: set[str] = set(agent.keys()) - {"max_steps", "mode"}
        if unknown_agent:
        ...
        if "mode" in agent:
            val = agent["mode"]
            if val not in ("react", "plan_execute"):
                raise SystemExit("Config error: agent.mode must be 'react' or 'plan_execute'")
            config.agent.mode = val
...
def _apply_env(config: KamaConfig) -> None:
    ...
    agent_mode = os.environ.get("KAMA_AGENT_MODE")
    if agent_mode is not None:
        if agent_mode not in ("react", "plan_execute"):
            raise SystemExit(
                "Config error: KAMA_AGENT_MODE must be 'react' or 'plan_execute',"
                f" got: {agent_mode!r}"
            )
