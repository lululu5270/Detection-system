...
class _SequenceProvider:
    # 鍒濆鍖栧搷搴斿簭鍒楀苟璁板綍璋冪敤娆℃暟
    def __init__(self, responses: list[LlmResponse]) -> None:
        self._responses = iter(responses)
        self.calls = 0

    # 杩斿洖涓嬩竴涓璁惧搷搴旓紝鐢ㄤ簬楠岃瘉 runner 璺敱鍒?plan_execute loop
    async def chat(
        self,
        messages: list[dict[str, object]],
        tool_schemas: list[dict[str, object]],
...
# 鍔熻兘锛氶獙璇?agent.mode=plan_execute 鏃?runner 浣跨敤 PlanExecuteLoop
# 璁捐锛氱敤涓ゅ段 provider 响应，断言会先 planning 再 execution，因此 chat 调用两次
async def test_runner_uses_plan_execute_mode(tmp_path: Path) -> None:
    cfg = _config()
    cfg.agent.mode = "plan_execute"
    provider = _SequenceProvider([
        LlmResponse(stop_reason="end_turn", text="1. Plan"),
        LlmResponse(stop_reason="end_turn", text="done"),
    ])
    runner = AgentRunner(cfg, provider=provider, runs_dir=tmp_path)

    outcome = await runner.run_and_capture("goal", run_id="run-plan")
